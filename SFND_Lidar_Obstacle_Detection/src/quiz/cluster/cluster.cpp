...
void fillCluster(const std::vector<std::vector<float>> points, uint index, KdTree *tree, const float &distanceTol, std::vector<bool> &processed, std::vector<uint> &cluster)
{

	processed[index] = true;
	cluster.push_back(index);
	std::vector<uint> nearby = tree->search(points[index], distanceTol);
	for (int idx : nearby)
	{
		if (!processed[idx])
		{
			fillCluster(points, idx, tree, distanceTol, processed, cluster);
		}
	}
}
std::vector<std::vector<uint>> euclideanCluster(const std::vector<std::vector<float>> points, KdTree *tree, float distanceTol)
{
	...
	while (i < points.size())
	{
		...
		fillCluster(points, i, tree, distanceTol, processed, cluster);
	...
	// 		fillCluster(points, index, tree, distanceTol, processed, cluster);
	// 		clusters.push_back(cluster);
	// 	}
	// }
...
int main()
{
	...
	std::vector<uint> nearby = tree->search({-6, 7}, 3.0);
	for (uint index : nearby)
		std::cout << index << ",";
	std::cout << std::endl;
	...
	//
	std::vector<std::vector<uint>> clusters = euclideanCluster(points, tree, 3.0);
	//
	...
	for (std::vector<uint> cluster : clusters)
	{
